...
# from cvnets.modules import InvertedResidual, gpeBlock
...
@MODEL_REGISTRY.register(name="moconv", type="classification")
class MoConv(BaseImageEncoder):
    """
    This class implements the `gpe architecture <https://arxiv.org/abs/2110.02178?context=cs.LG>`_
    """
    ...
    def __init__(self, opts, *args, **kwargs) -> None:
        ...
        gpe_config = get_configuration(opts=opts)
        ...
        self.layer_1, out_channels = self._make_layer(
            opts=opts, input_channel=in_channels, cfg=gpe_config["layer1"]
        )
        self.model_conf_dict["layer1"] = {"in": in_channels, "out": out_channels}

        in_channels = out_channels
        self.layer_2, out_channels = self._make_layer(
            opts=opts, input_channel=in_channels, cfg=gpe_config["layer2"]
        )
        self.model_conf_dict["layer2"] = {"in": in_channels, "out": out_channels}

        in_channels = out_channels
        self.layer_3, out_channels = self._make_layer(
            opts=opts, input_channel=in_channels, cfg=gpe_config["layer3"]
        )
        ...
        self.layer_5, out_channels = self._make_layer(
            opts=opts,
            input_channel=in_channels,
            cfg=gpe_config["layer5"],
            dilate=self.dilate_l5,
        )
        self.model_conf_dict["layer5"] = {"in": in_channels, "out": out_channels}

        in_channels = out_channels
        exp_channels = min(gpe_config["last_layer_exp_factor"] * in_channels, 960)
    ...
    def _make_layer(
        ...
        **kwargs
        ...
        block_type = cfg.get("block_type", "gpe")
        if block_type.lower() == "gpe":
